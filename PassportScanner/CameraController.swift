...
extension CameraController {
    ...
    func readDocument(_ document: TravelDocument) {
        if NFCTagReaderSession.readingAvailable {
            readerSession.read(with: document.mrzKey, dataGroups: [.DG1, .DG2]) { (groups, error) in
                if let error = error { print(error); self.isPaused = false; return }
                guard let dg1 = groups?[0] as? DG1 else { return }
                guard let dg2 = groups?[1] as? DG2 else { return }
                
                self.scannerSquare.isHidden = true
                
                let controller = DisplayController(name: dg1.fullName, dob: dg1.elements["5F57"]!, image: dg2.getImage()!)
                controller.dismissAction = { [weak self] in
                    self?.scannerSquare.isHidden = false
                    self?.isPaused = false
                }
